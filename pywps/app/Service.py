...
class Service(object):
    ...
    def execute(self, identifier, wps_request, uuid):
        ...
        process = self.prepare_process_for_execution(identifier, wps_request)

        response = self._parse_and_execute(process, wps_request, uuid)

        return response

    def prepare_process_for_execution(self, identifier, wps_request):
        ...
        if not wps_request.raw:
            for output_id in wps_request.outputs:
                as_reference = wps_request.outputs[output_id].get('asReference', 'false')
                as_reference = as_reference.lower() == 'true'
                for output in process.outputs:
                    if output.identifier == output_id:
                        output.as_reference = as_reference
                        break
